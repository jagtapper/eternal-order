...
export function generateStaticParams() {
  return getSlugs("now").map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const article = await getArticle("now", slug);
  if (!article) return {};
  return {
    title: article.title,
    ...
    openGraph: {
      ...
      url: `${site.url}/now/${slug}`,
    },
  };
}

export default async function NowArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const related = getAll("digest").slice(0, 2);
